...
npx tsx apps/cli/src/main.ts config add-provider webdav \
...
```bash
tn config add-provider webdav --url <url> --username <user> --password <pass>
tn config add-provider s3 --bucket <bucket> --endpoint <endpoint> \
  --access-key-id <id> --secret-access-key <key>
tn config list-providers
tn config remove-provider <id>
```

### Clone Notebook

```bash
tn clone <providerId>:<remotePath> [dir-name]
```

### 管理 Remote

```bash
tn remote set origin <providerId>:<remotePath>
