...
| `FsVolumeCredential` | Volume + Credentials | 怎么连 | per-device |
...
{Scheme}VolumeCredential = {Scheme}Volume & {Scheme}Credentials
{Scheme}ClientConfig  = {Scheme}Volume & {Scheme}Credentials & { rootPath }
```

跨 scheme 联合类型（`FsVolume`、`FsEndpoint`、`FsVolumeCredential`、`FsClientConfig`）在 `types.ts` 中定义，每增加 scheme 时扩展对应 union。
...
`volumeUrl` 用作 `FsVolumeCredentialStore` 的 key。
...
| 前缀 | 含义 | 示例 |
|------|------|------|
| `Fs` | 模块级，跨 scheme | `FsClient`、`FsScheme`、`FsVolumeCredential` |
...
## 怎么连：VolumeCredential

VolumeCredential（凭证）持久化在 `FsVolumeCredentialStore` 中，per-device 全局配置（不在 vault 中），以 `volumeUrl` 为 key。
