...
import { Globe, ArrowLeft, Folder, FileText, Trash2, Save, Plus } from "lucide-react";
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-6 space-y-6 text-zinc-900 dark:text-zinc-50">
      <div className="max-w-4xl mx-auto space-y-6">
        <header className="flex items-center gap-4">
            ...
            <div className="bg-blue-600 p-2 rounded-lg text-white"><Globe className="w-6 h-6" /></div>
            <div>
        ...
        <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="sm:col-span-3">
                    <Button onClick={() => handleList("/")} disabled={status === "loading"}>Connect & List Root</Button>
                </div>
            </CardContent>
        </Card>
            ...
            <Card className="h-[600px] flex flex-col">
                <CardHeader className="pb-2">
                    <CardTitle className="flex justify-between items-center">
                        ...
                        <div className="flex gap-1">
                            <Button size="icon" variant="ghost" onClick={handleMkdir}><Plus className="w-4 h-4"/></Button>
                            <Button size="icon" variant="ghost" onClick={() => handleList(currentPath)}>↺</Button>
                        </div>
                    </CardTitle>
                    <CardDescription className="truncate" title={currentPath}>{currentPath}</CardDescription>
                </CardHeader>
                <CardContent className="flex-1 overflow-auto p-0">
                    <div className="divide-y">
                        ...
                        {items.map((item) => (
                            <div key={item.filename} className="group flex items-center justify-between p-3 hover:bg-muted text-sm">
                                <button 
                                    className="flex items-center gap-2 flex-1 truncate text-left"
                                    onClick={() => item.type === "directory" ? handleList(item.filename) : handleRead(item)}
                                >
                                    {item.type === "directory" ? <Folder className="w-4 h-4 text-blue-500" /> : <FileText className="w-4 h-4 text-zinc-500" />}
